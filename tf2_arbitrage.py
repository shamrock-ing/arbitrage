#!/usr/bin/env python3
"""
TF2 Arbitrage Bot с анализом рентабельности апгрейдов
Поддерживает все типы killstreak и australium предметов
"""
...
class UpgradeArbitrage:
	...
	async def analyze_upgrade_profitability(self, page: Page, base_item: str, kit_type: str = "specialized"):
		...
		try:
			...
			# Правильное название: "Strange Specialized Killstreak Rocket Launcher" (не "Strange Specialized Killstreak Kit Rocket Launcher")
			if kit_type == "specialized":
				upgraded_item_name = f"Strange Specialized Killstreak {base_item}"
			elif kit_type == "professional":
				upgraded_item_name = f"Strange Professional Killstreak {base_item}"
			else:
